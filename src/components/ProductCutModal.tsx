...
export const ProductCutModal: React.FC<ProductCutModalProps> = ({
  ...
  // Weight variations handling (Requirements 2 & 3)
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.organVariations && product.organVariations.length > 0) {
        setSelectedOrganVarId(''); // Customer must choose variation
      }
    }
  }, [product?.id, isOpen]);
  ...
  const currentPackagingObj = PACKAGING_OPTIONS.find((p) => p.id === packaging) || PACKAGING_OPTIONS[0];
  // Packaging fee for cooler box is applied per order at checkout (RM5 for <10 units, RM10 for 10-30 units)
  ...
  const handleConfirm = () => {
    if (quantity > 30) {
      setValidationError('Maksimum pesanan adalah 30 unit bagi satu trip penghantaran.');
      return;
    }
    if (product.hasOrganVariations && !selectedOrganVarId) {
      setValidationError('Sila pilih jenis produk (Hati Sahaja, Pedal Sahaja, atau Campur Hati & Pedal) terlebih dahulu.');
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-stone-950/70 backdrop-blur-xs overflow-y-auto animate-fade-in">
      ...
      >
          ...
          <div className="flex items-center gap-4 pr-8">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                {product.originalPrice && product.originalPrice > baseUnitPrice && (
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-rose-500/20 text-rose-300 border border-rose-500/40">
                    Jimat RM {(product.originalPrice - baseUnitPrice).toFixed(2)}
                  </span>
                )}
              </div>
              ...
              <p className="text-xs text-stone-400 mt-0.5">
                {activeWeightOpt
                  ? `Pilihan Berat: ${activeWeightOpt.weightLabel} (RM ${activeWeightOpt.price.toFixed(2)}) • Sedia ada: ${activeWeightOpt.availableStock} unit`
                  : activeOrganVar 
                  ? `Variasi: ${activeOrganVar.label} • RM ${activeOrganVar.price.toFixed(2)} / ${activeOrganVar.unit}`
                  : product.subtitle}
              </p>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-6 flex-1 text-stone-800 dark:text-stone-200">
          ...
          {/* STEP 1: PILIHAN BERAT AYAM & KUANTITI AVAILABLE (REQUIREMENTS 2 & 3) */}
            ...
            <div className="bg-emerald-50/70 dark:bg-emerald-950/40 border-2 border-emerald-200 dark:border-emerald-800/80 p-4 sm:p-5 rounded-2xl">
              <div className="flex items-center justify-between mb-3">
                <label className="flex items-center gap-2 text-sm font-extrabold text-stone-900 dark:text-white uppercase tracking-wide">
                  ...
                  <span>1. Pilih Saiz / Berat Ayam</span>
                </label>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-2 pt-2 border-t border-stone-100 dark:border-stone-700/60 flex items-center justify-between text-[11px]">
                        <span className="text-stone-500 dark:text-stone-400 font-medium">Baki Stok:</span>
                        <span className={`font-black px-2 py-0.5 rounded-md ${
                          isOptAvailable
                            ? 'bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300'
                            : 'bg-rose-100 dark:bg-rose-950 text-rose-700 dark:text-rose-400'
                        }`}>
                          {isOptAvailable ? `${opt.availableStock} ekor` : 'Habis'}
                        </span>
                      </div>
                    </div>
                  );
            ...
            <div className={`p-4 rounded-2xl border-2 transition-all ${
              ...
              <div className="flex items-center justify-between mb-3">
                <label className="flex items-center gap-2 text-sm font-extrabold text-stone-900 dark:text-white uppercase tracking-wide">
                  ...
                  <span>Pilih Variasi Hati / Pedal (Wajib Pilih)</span>
                </label>
                <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${
                  ...
                  {selectedOrganVarId ? 'Telah Dipilih' : 'Wajib Pilih 1'}
                </span>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-base font-black text-emerald-700 dark:text-emerald-400">
                        ...
                        <span className="text-xs font-normal text-stone-500 dark:text-stone-400">/ {v.unit}</span>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <p className="text-xs text-stone-600 dark:text-stone-400 mt-1">
                    Bulu dibuang bersih secara standard mesin rendaman air panas tanpa pembakaran.
                  </p>
                </div>
                ...
                >
                  ...
                  <p className="text-xs text-stone-600 dark:text-stone-400 mt-1">
                    Dibakar tradisional dengan api bagi menanggalkan bulu halus & menambah aroma sedap rendang.
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* JENIS POTONGAN (CHICKEN CUTS) - HANYA UNTUK PRODUK YANG MENYOKONG POTONGAN */}
          {product.supportsCutting && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className="flex items-center gap-2 text-sm font-extrabold text-stone-900 dark:text-white uppercase tracking-wide">
                  <Scissors className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                  <span>Pilih Gaya Potongan</span>
                </label>
          ...
          {/* SERVIS PEMBERSIHAN KHAS - HANYA UNTUK PRODUK YANG MEMBENARKAN PEMBERSIHAN KHAS */}
          {product.supportsCleaning !== false && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className="flex items-center gap-2 text-sm font-extrabold text-stone-900 dark:text-white uppercase tracking-wide">
                  <Sparkles className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                  <span>Servis Pembersihan Khas (Boleh Pilih Lebih)</span>
                </label>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">
                        {clean.price === 0 ? 'Percuma' : `+RM ${(clean.price || 0).toFixed(2)}`}
                      </span>
                    </div>
                  );
                })}
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="flex items-center gap-2 text-sm font-extrabold text-stone-900 dark:text-white uppercase tracking-wide">
                ...
                <span>4. Pilihan Pembungkusan Kesegaran</span>
              </label>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 text-xs font-bold text-emerald-600 dark:text-emerald-400">
                      {pack.price === 0 ? 'Percuma' : `+RM ${pack.price.toFixed(2)}`}
                    </div>
                  </div>
                );
              })}
            ...
            {packaging === 'vacuum-pack' && (
              <div className="mt-3 p-3 rounded-xl bg-amber-50/80 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 flex items-start gap-2.5 text-xs text-amber-900 dark:text-amber-200 animate-fade-in">
                ...
                <p className="leading-relaxed">
                  <strong className="font-bold">Kelebihan Pek Kedap Udara:</strong> Menghalang pembentukan fros ais (freezer burn), mengunci kesegaran jus ayam sehingga 3x lebih lama di dalam peti sejuk beku.
                </p>
              </div>
            )}
            ...
            {packaging === 'cooler-box' && (
              <div className="mt-3 p-3 rounded-xl bg-blue-50/80 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 flex items-start gap-2.5 text-xs text-blue-900 dark:text-blue-200 animate-fade-in">
                ...
                <div className="leading-relaxed space-y-0.5">
                  <p><strong className="font-bold">Perlindungan Kotak Penebat Cooler Box:</strong> Ayam segar dimuatkan ke dalam kotak bertebat ais bertutup khas bagi mengekalkan suhu beku 0°C sepanjang perjalanan.</p>
                  <p className="text-[11px] font-semibold text-blue-700 dark:text-blue-300">★ Kadar Caj: +RM5.00 (kurang 10 unit) | +RM10.00 (10 - 30 unit) setiap penghantaran.</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-stone-700 dark:text-stone-300 uppercase tracking-wider mb-1.5">
              Nota Khas Untuk Tukang Potong (Pilihan):
            </label>
            <input
              type="text"
              value={specialNotes}
              onChange={(e) => setSpecialNotes(e.target.value)}
              placeholder="Contoh: Tolong asingkan kaki, potong buang lemak lebihan, atau bungkus berasingan..."
        ...
        <div className="p-4 sm:p-5 bg-stone-50 dark:bg-stone-950 border-t border-stone-200 dark:border-stone-800 shrink-0 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto justify-between sm:justify-start">
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-stone-600 dark:text-stone-400 uppercase">Kuantiti:</span>
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
            <div className="text-right">
              <span className="text-[11px] text-stone-500 dark:text-stone-400 block">Jumlah Harga</span>
